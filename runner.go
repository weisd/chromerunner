// package chromerunner provides a Chrome process runner.
...
const (
	...
	DefaultUserDataDirPrefix = "chrome-runner.%d."
)
...
func (r *Runner) Shutdown(ctxt context.Context, opts ...client.Option) error {
	...
	targets, err := r.ListTargets(ctxt)
	if err != nil {
		return err
	}
	...
	for i, t := range targets {
		wg.Add(1)
		go func(wg *sync.WaitGroup, i int, t *Target) {
			defer wg.Done()
			errs[i] = r.CloseTarget(ctxt, t)
		}(wg, i, t)
	}
	wg.Wait()
...
func (r *Runner) httpGet(ctx context.Context, path string) (*http.Response, error) {
	...
	req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("http://localhost:%d%s", r.Port(), path), nil)
	if err != nil {
		return nil, err
	}
...
// ListTargets returns a list with all devtools Targets.
func (r *Runner) ListTargets(ctx context.Context) ([]*Target, error) {
	resp, err := r.httpGet(ctx, "/json/list")
	if err != nil {
		return nil, err
	}
...
// CloseTarget the Target.
func (r *Runner) CloseTarget(ctx context.Context, t *Target) error {
	resp, err := r.httpGet(ctx, "/json/close/"+t.ID)
	if err != nil {
		return err
	}
	defer resp.Body.Close()
